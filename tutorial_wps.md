...
Este tutorial explica como preparar dados do ERA5 para o `ungrib.exe` do WPS (WRF Pre-Processing System) e gerar arquivos no formato intermediário do WPS, como:
...
2. preparar um diretório de trabalho para o `ungrib`;
...
## 1. Entendendo o papel da `Vtable`

Antes de qualquer execução do `ungrib`, é importante entender a `Vtable`.
...
Ela deve estar presente no diretório de trabalho do `ungrib` com o nome exato `Vtable`. Isso normalmente é feito com um link simbólico:
...
Crie um diretório de trabalho vazio para rodar o `ungrib`. Nesse diretório, você deve ter:
...
cp /caminho/para/WPS/link_grib.csh link_grib.csh
ln -s /caminho/para/WPS/ungrib.exe ungrib.exe
```

## 4. Preparação do `namelist.wps`
...
Nesse contexto, os blocos `geogrid` e `metgrid` estão presente apenas para satisfazer o formato do `namelist.wps`. Estes dois blocos são utilizados para, respectivamente, criar o domínio do WRF e interpolar a IC e LBC no domínio gerado. Já o campo `ungrib` é importante para executar a etapa do "desgribar" o dado, e `share` são é entre os diferentes executáveis de pré-processamento do WRF.
...
Se os arquivos GRIB estiverem em outro diretório, a execução típica é:

```bash
/caminho/para/WPS/link_grib.csh /caminho/para/gribs/*.grib
```

Depois disso, o diretório de trabalho do `ungrib` passa a conter os `GRIBFILE.*`.
...
E então converte os GRIBs para o formato intermediário do WPS, que serão utilizados entradas para o `init_atmosphere` do MONAN/MPAS para criação de IC e LBC.

A execução típica é:

```bash
/caminho/para/wps_work/ungrib.exe
```
